...
def _handle_path(path, sess):
    if path.endswith('.npz'):
        ...
        m, s, _ = calculate_activation_statistics(x, sess)
    return m, s
...
def calculate_activation(data):
    ...
    with tf.Session() as sess:
        ...
        mu, sigma, act = calculate_activation_statistics(data, sess, batch_size=100)
    return mu, sigma, act
...
if __name__ == "__main__":
    ...
    parser.add_argument("--real_data", type=str, default='mnist',
                        help='dataset name /path to the real image data /to .npz statistic files')
